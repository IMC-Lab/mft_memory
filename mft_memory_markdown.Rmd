...
hist(data$moral_decision, xlim = c(1,5))
...
modelRetRT2 <- glmer(retrieval_RT ~ category + (1 | subID) + (1 | vigfile), data=data_old, family = Gamma(link='identity'), start=list(rho=0))

modelRetRT3 <- glmer(retrieval_RT ~ category + (1 | subID) + (1 | vigfile), data=data_old, family = inverse.gaussian(link = 'identity'), start=list(rho=0))
