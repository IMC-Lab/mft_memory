...
# model comparisons: relevance of moral decision for hits
modelOld1 <- glmer(correct ~ category + (1 + category | subID),
               data=data_old, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=50000)))
modelOld2 <- glmer(correct ~ category + moral_decision + (1 + category | subID),
               data=data_old, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=50000)))
modelOld3 <- glmer(correct ~ category * moral_decision + (1 + category | subID),
               data=data_old, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=50000)))
anova(modelOld1, modelOld2, modelOld3) #model2 --> winner! 

modelOld4 <- glmer(correct ~ category + (1 | subID),
               data=data_old, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=50000)))
modelOld5 <- glmer(correct ~ category + (1 + moral_decision | subID),
               data=data_old, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=50000)))
modelOld6 <- glmer(correct ~ category + (1 + moral_decision + category | subID),
               data=data_old, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=50000)))
modelOld7 <- glmer(correct ~ category + (1 + moral_decision * category | subID),
               data=data_old, family=binomial(link = 'logit'), control=glmerControl())
anova(modelOld4, modelOld5, modelOld2, modelOld6, modelOld7)

# plot
data_old <- data_old %>% 
            group_by(subID, category, moral_decision) %>%
            mutate(propCorrect = mean(correct, na.rm = TRUE))
  
modelOld1 %>% emmeans(~ category * moral_decision, type='response') %>% as.data.frame %>%
  ggplot(aes(x=moral_decision, y=prob)) +
  geom_pointrange(aes(ymax=asymp.LCL, ymin=asymp.UCL), alpha=1, fatten=5, color='black') +
  facet_wrap(~category, nrow=2) +
  geom_violin(aes(x=moral_decision, y=propCorrect, color=category, fill=category), data=data_old, alpha=0.4, scale='count') +
  ggtitle('Hits for old vignettes (n=30)') + xlab('Moral Decision (not morally wrong-extremely morally wrong)')
