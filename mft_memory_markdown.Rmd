...
modelRetRT <- lmer(log_retrieval_RT ~ category + (1 | subID) + (1 | vigfile), data=data_old)
...
ggplot(data, aes(x=confresp, color=category, fill=category)) +
  ...
  ggtitle('Distribution of Confidence Judgments by Old/New') +
...
modelConfidence %>% emmeans(~ category * old * correct, type='response') %>% as.data.frame %>%
  ...
  facet_grid(correct~old, labeller = labeller(correct=as_labeller(confLabels), old=as_labeller(memLabels))) +
  geom_violin(aes(x=category, y=averageConf, color=superordinate, fill=superordinate), data=data, alpha=0.7, scale='count', bw=0.35) +
  geom_pointrange(aes(ymax=asymp.LCL, ymin=asymp.UCL), alpha=1, fatten=2, color='black') +
  theme(strip.text.x = element_text(size=10, face='bold')) +
