...
secs_ret <- subset(secs, subID %in% ret_all$subID) %>% filter(!is.nan(SocialConservatism))
...
emorate <- emorate[2:nrow(emorate),] %>% pivot_longer(2:ncol(emorate), names_to=c('vigfile', 'emotion'), names_pattern='(.*)\\.(.*)', values_to='emoresp')
emorate$emotion[emorate$emotion == '1'] = 'angry'
emorate$emotion[emorate$emotion == '2'] = 'sad'
emorate$emotion[emorate$emotion == '3'] = 'disgusted'
emorate$emotion[emorate$emotion == '4'] = 'afraid'
emorate$emotion[emorate$emotion == '5'] = 'contemptuous'
emorate$emotion[emorate$emotion == '6'] = 'amused'
emorate$emoresp <- as.numeric(emorate$emoresp)
emorate_ret <- subset(emorate, subID %in% ret_all$subID) %>% filter(!is.na(emotion))
