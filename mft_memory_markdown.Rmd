...
earlyencodingfiles = list.files('C:/Users/imclab/Desktop/MFT re-analysis/data/encoding/old', full.names = TRUE, pattern = '.csv', recursive = FALSE)
...
testOld <- ret_all %>% group_by(subID) %>% summarise(confmean = mean(as.numeric(confresp), na.rm = TRUE))

all_surveys$subID <- as.numeric(all_surveys$subID)
test$subID %in% all_surveys$subID
# ppts in test that are not in qualtrics: 23; 20 is in there but they stopped responding after MFQ

testOld$subID %in% all_surveys$subID
...
ret_old <- subset(data, old == 1) %>% subset(subID %in% mfq_ret$subID)
ret_new <- subset(data, old == 0) %>% subset(subID %in% mfq_ret$subID)
ret_old_qualtrics <- data %>% subset(old == 1) %>% subset(subID %in% mfq_ret$subID)
ret_new_qualtrics <- data %>% subset(old == 0) %>% subset(subID %in% mfq_ret$subID)
ret_old_qualtrics <- left_join(ret_old_qualtrics, mfq_ret, by='subID')
ret_old_qualtrics <- left_join(ret_old_qualtrics, secs_ret, by='subID')
ret_new_qualtrics <- left_join(ret_new_qualtrics, mfq_ret, by='subID')
ret_new_qualtrics <- left_join(ret_new_qualtrics, secs_ret, by='subID')

count(ret_old_qualtrics, SocialConservatism > 50)
count(ret_old_qualtrics, EconomicConservatism > 50)
ret_old_qualtrics$socially_conservative[ret_old_qualtrics$SocialConservatism > 50] = 1
ret_old_qualtrics$socially_conservative[ret_old_qualtrics$SocialConservatism < 50] = 0
ret_old_qualtrics$fiscally_conservative[ret_old_qualtrics$EconomicConservatism > 50] = 1
ret_old_qualtrics$fiscally_conservative[ret_old_qualtrics$EconomicConservatism < 50] = 0

ret_old$confresp <- as.factor(as.numeric(ret_old$confresp))
ret_new$confresp <- as.factor(as.numeric(ret_new$confresp))
ret_old_qualtrics$confresp <- as.factor(as.numeric(ret_old_qualtrics$confresp))
ret_new_qualtrics$confresp <- as.factor(as.numeric(ret_new_qualtrics$confresp))

# predicting correct using category and moral decision
modelOld1 <- glmer(correct ~ moral_decision + category + (1 + moral_decision | subID),
               data=ret_old_qualtrics, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=50000)))

modelOld2 <- glmer(correct ~ moral_decision + category + socially_conservative + fiscally_conservative + (1 + moral_decision + careScore + fairnessScore + loyaltyScore + authorityScore + purityScore + SocialConservatism + EconomicConservatism | subID), data = ret_old_qualtrics, family = binomial(link = 'logit'), control=glmerControl(optCtrl = list(maxfun=50000)))
...
# plot predictions
modelOld1 %>% ggpredict(terms=c('moral_decision', 'category')) %>%
...
modelOld1 %>% ggpredict(terms=c('moral_decision')) %>%
...
modelNew <- glmer(correct ~ category + (1 + category | subID), data=ret_new, family=binomial(link = 'logit'), control=glmerControl(optCtrl=list(maxfun=25000)))
summary(modelNew)
plot(modelNew)
ggpredict()
