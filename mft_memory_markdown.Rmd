...
for (c in data$category) {
  i = i + 1
  if (strcmp(c, 'Care-Emo') == TRUE)
    data$superordinate[i] = 'Individualizing'
  else if (strcmp(c, 'Care-Phys') == TRUE)
    data$superordinate[i] = 'Individualizing'
  else if (strcmp(c, 'Fairness') == TRUE)
    data$superordinate[i] = 'Individualizing'
  else if (strcmp(c, 'Loyalty') == TRUE)
    data$superordinate[i] = 'Binding'
  else if (strcmp(c, 'Authority') == TRUE)
    data$superordinate[i] = 'Binding'
  else if (strcmp(c, 'Purity') == TRUE)
    data$superordinate[i] = 'Binding'
  else
    data$superordinate[i] = data$category[i]
}
...
data <- data %>% filter(retrieval_RT != 0)
data <- data %>% filter(confRT != 0)
data$moral_decision <- as.numeric(data$moral_decision)
data$confresp <- as.numeric(data$confresp)
data_old <- subset(data, old == '1')
...
category_means <- data_old %>% group_by(category, subID) %>% summarise(mean = mean(as.integer(moral_decision))) 
...
emmeans(modelMoral, ~ category) %>% as.data.frame() %>% 
  mutate(fct_relevel(category, c('Authority', 'Loyalty', 'Purity', 'Care-Emo', 'Care-Phys', 'Fairness', 'Liberty', 'Social Norms'))) %>%
  ggplot(aes(x=category, y=emmean)) + ylim(1,4) + ylab('Estimated marginal mean') +
  geom_violin(data=data_old, mapping=aes(y=moral_decision, fill=superordinate), alpha=0.7, bw=0.45) + 
  geom_pointrange(aes(ymin=asymp.LCL, ymax=asymp.UCL), alpha=1, fatten=5, color='black') +
  ggtitle('Model estimates: moral judgment') 
...
# visualize
hist(data_old$encoding_RT)
data_old <- data_old %>% filter(encoding_RT > 0.01)
...
modelMoralRT %>% emmeans(~ category) %>% as.data.frame() %>% 
  ...
  geom_violin(data=data_old, mapping=aes(y=encoding_RT, fill = category, color=category), alpha=0.7) +
  geom_pointrange(aes(ymax=asymp.LCL, ymin=asymp.UCL), alpha=1, fatten=3, color = 'black', size=1) +
  ylab('Reaction time (seconds)') +
  ggtitle('Moral judgment reaction time (model estimates + raw data)') 

modelMoralRT %>% emmeans(~ category) %>% as.data.frame() %>% 
  ggplot(aes(x=category, y=emmean)) +
  geom_pointrange(aes(ymax=asymp.LCL, ymin=asymp.UCL, color=category), alpha=1, fatten=8) +
  ylab('Reaction time (seconds)') +
...
modelMemory <- glmer(correct ~ category + old + (1 + category | subID),
...
modelMemory %>% emmeans(~ category * old, type='response') %>% as.data.frame %>%
  ...
  geom_violin(aes(x=category, y=propCorrect, color=superordinate, fill=superordinate), data=data, alpha=0.7, scale='count') +
...
modelConfidence %>% emmeans(~ category * old * correct, type='response') %>% as.data.frame %>%
  ...
  geom_pointrange(aes(ymax=asymp.LCL, ymin=asymp.UCL, color=correct), alpha=1, fatten=3, position=position_dodge(1)) + scale_color_manual(values = c("white", "black")) +
  theme(strip.text.x = element_text(size=10, face='bold'), strip.text.y = element_text(size=10, face='bold')) +
