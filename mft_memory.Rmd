...
library(patchwork)
library(performance)
library(eeptools) #note: this is not compatible with R version 4.0.0 as of 05/12/2020
library(ggcorrplot)
library(extrafont)
library(purrr)
library(parameters)
...
# functions for running superordinate contrasts
...
mfv_comp <- data_old %>% select(category, vigfile, subID, moral_decision) %>%
  mutate(moral_decision = scale(moral_decision)) %>%
  group_by(vigfile) %>%
  summarise(moral_decision = mean(moral_decision))
# test for normality
shapiro.test(mfv$moral_decision)
shapiro.test(mfv_comp$moral_decision)
# run a t-test anyway (ordinal data is hardly ever normal) & wilcox test for extra validity
t.test(mfv$moral_decision, mfv_comp$moral_decision, var.eq = T)
wilcox.test(mfv$moral_decision, mfv_comp$moral_decision, var.eq = T)
...
model_parameters(modelMoral)
model_parameters(modelMoral, standardize = 'refit')
...
# contrasts
modelMoral.supContrast <- emmeans(modelMoral, ~ category) %>% contrast(SuperordinateContrasts, adjust='bonferroni')
eff_size(modelMoral.supContrast, sigma = modelMoral.sigma, edf = df.residual(modelMoral), method = 'identity')

modelMoral.newContrast <- emmeans(modelMoral, ~ category) %>% contrast(newSuperordinateContrasts, adjust='bonferroni')
eff_size(modelMoral.newContrast, sigma = modelMoral.sigma, edf = df.residual(modelMoral), method = 'identity')
...
p1 <- ggplot(data = mean_centered_results_LV1, 
  ...
  ggtitle('LV1: 24.4% crossblock variance')
...
compare_performance(superordinateNull, modelMoral, superordinateOG, superordinateNew, bayesfactor = FALSE, rank = TRUE)
```
...
compare_performance(em_null_moral, em_modelMoral, em_only_moral, em_add_moral, em_int_moral, bayesfactor = FALSE, rank = TRUE)
```
...
#   return(emmeans(model, ~ em_measure, at=list_modify(em_add.cov, em_measure=c(0,1))) %>%
#     eff_size(sigma = sd(em_compare$dv), edf = df.residual(model), method = 'revpairwise'))
# }

emmeans(em_add_moral, ~ angry, at=list_modify(em_add_moral.cov, angry=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ sad, at=list_modify(em_add_moral.cov, sad=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ disgusted, at=list_modify(em_add_moral.cov, disgusted=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ afraid, at=list_modify(em_add_moral.cov, afraid=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ contemptuous, at=list_modify(em_add_moral.cov, contemptuous=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ amused, at=list_modify(em_add_moral.cov, amused=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ PersonalDistressScore, at=list_modify(em_add_moral.cov, PersonalDistressScore=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ EmpathicConcernScore, at=list_modify(em_add_moral.cov, EmpathicConcernScore=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ PerspectiveTakingScore, at=list_modify(em_add_moral.cov, PerspectiveTakingScore=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')

emmeans(em_add_moral, ~ FantasyScore, at=list_modify(em_add_moral.cov, FantasyScore=c(0,1))) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_add_moral), method = 'revpairwise')
```
...
memory_supContrast.df <- emmeans(modelMemory, ~ old * category, type = 'response', lmer.df = 'satterthwaite') %>% 
  contrast(interaction = c("pairwise", old_superordinate, by = NULL), adjust='bonferroni') %>% as.data.frame() %>%
  mutate(odds_ratio = exp(estimate))

memory_supContrastNew.df <- emmeans(modelMemory, ~ old * category, type = 'response', lmer.df = 'satterthwaite') %>% 
  contrast(interaction = c("pairwise", new_superordinate, by = NULL), adjust='bonferroni') %>% as.data.frame() %>%
  mutate(odds_ratio = exp(estimate))
...
em_comp_memory.df <- compare_performance(em_null_memory, em_modelMemory, em_only_memory, em_add_memory, em_int_memory, 
...
write.csv(em_comp_memory.df, file = 'figures/paper/table_emCompMemory.csv', row.names = FALSE)
```
...
# compute effect size -- update after tomorrow's call with Felipe
emmeans(modelConfidence, ~ category) %>%
  eff_size(sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'eff')

emmeans(modelConfidence, ~ old) %>%
  eff_size(sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'eff')

emmeans(modelConfidence, ~ correct) %>%
  eff_size(sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'eff')

emmeans(modelConfidence, eff ~ correct*old) %>%
  eff_size(sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'eff')

emmeans(modelConfidence, ~ correct*category) %>%
  eff_size(sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'eff')

emmeans(modelConfidence, ~ category*old) %>%
  eff_size(sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'eff')

emmeans(modelConfidence, ~ correct*old*category) %>%
  eff_size(sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'eff')
...
supContrastConf <- modelConfidence %>% 
  ...
  contrast(interaction=c('pairwise', 'pairwise', old_superordinate), adjust = 'bonferroni')

supContrastConf.eff <- eff_size(supContrastConf, sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'identity') %>%
  as.data.frame() %>% mutate(effect.size = round(effect.size, 2))

supContrastConf.df <- as.data.frame(supContrastConf) %>%
  mutate_at(c('estimate', 'SE', 'df', 't.ratio', 'p.value'), functions(round(.,2)))
...
supContrastNewConf <- modelConfidence %>% 
  emmeans(~ old * correct * category) %>%
    contrast(interaction=c('pairwise', 'pairwise', new_superordinate), adjust = 'bonferroni')

supContrastNewConf.eff <- eff_size(supContrastNewConf, sigma = sd(data$confresp), edf = df.residual(modelConfidence), method = 'identity') %>%
  as.data.frame() %>% mutate(effect.size = round(effect.size, 2))

supContrastNewConf.df <- as.data.frame(supContrastNewConf) %>%
  mutate_at(c('estimate', 'SE', 'df', 't.ratio', 'p.value'), funs(round(.,2)))

supContrastNewConf.df$eff_size <- supContrastNewConf.eff$effect.size 
write.csv(supContrastNewConf.df, file = 'figures/paper/table_supContrastNewConf.csv', row.names = F)
...
# emotion_rating <- read.csv('data/qualtrics/emotion_rating.csv', stringsAsFactors = FALSE)
# all_other_surveys <- read.csv('data/qualtrics/all_other_surveys.csv', stringsAsFactors = FALSE)
...
# tidy emotion_rating
# emotion_rating <- read.csv('C:/Users/imclab/Desktop/MFT re-analysis/data/qualtrics/emotion_rating.csv', stringsAsFactors = FALSE)
# emotion_rating[2:30, 2:721] <- as.data.frame(sapply(emotion_rating[2:30, 2:721], function(s) substr(s, 1, 2)), stringsAsFactors = FALSE)
# names(emotion_rating)[1] <- 'subID'
# emotion_rating <- emotion_rating[2:nrow(emotion_rating),] %>% pivot_longer(2:ncol(emotion_rating), names_to=c('vigfile', 'emotion'), names_pattern='([a-z]+\\d+)\\.*(.*)' , values_to='emoresp')
# emotion_rating$emotion[emotion_rating$emotion == ''] = 'angry'
...
# emotion_rating$emotion[emotion_rating$emotion == '5'] = 'amused'
# emotion_rating$emoresp <- as.numeric(emotion_rating$emoresp)
# emotion_rating_ret <- subset(emotion_rating, subID %in% ret_all$subID)
# 
