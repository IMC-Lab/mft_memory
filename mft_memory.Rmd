...
# plot amygdala voxel response during LV2
amy_LV2 <- read.csv('data/PLS/mean-centered/all_conditions/LV2_amy_responseFunction.csv', header=F)
colnames(amy_LV2) <- paste0('TR', 1:7)
amy_LV2$subID <- rep(1:27, each = 8)
amy_LV2$category <- matlab_categories
amy_LV2 <- pivot_longer(amy_LV2, cols=starts_with('TR'), names_to='TR', values_to='signal')

amy_LV2 %>%
lmer(signal ~ TR * category + (1 | subID), .) %>%
    emmeans(~ TR * category) %>% as.data.frame() %>%
    mutate(superordinate = superordinateCategory(category),
           category = factor(category, levels = plotting_categories)) %>%
    ggplot(aes(x=TR, y=emmean, group=category, color=category, fill=category))  + 
    scale_x_discrete(expand = c(0, 0, 0, 0.2)) +
    geom_hline(aes(yintercept = 0), size=1) +
    geom_line(size=1) + 
    geom_point(aes(fill=category, color=category, shape = category), size=3) +
    scale_shape_manual(values = c(4,15,16,17,0,1,2, 3)) +
    ...
    labs(y = 'BOLD signal', color='Foundation', fill='Foundation', shape='Foundation', title='MC-LV2: Amygdala response function')
ggsave('../additionalFigures/LV2_amy_responseFunction.png', dpi='retina', width=8)
