...
#font_import(paths = NULL, recursive = TRUE, prompt = TRUE,pattern = NULL)
>>>>>>> 6df7b9f47a07458cc47cb498520b9e215ecd5abe
...
# normed data from Clifford et al (2015)
mfv <- read.csv('data/mfv.csv')
mfv <- mfv %>% group_by(category) %>%
  mutate(moral_decision = scale(moral_decision))
mfv_comp <- data_old %>% 
  select(category, vigfile, moral_decision) %>%
  mutate(moral_decision = scale(moral_decision)) %>%
  group_by(vigfile) %>% 
  summarise(moral_decision = mean(moral_decision))
t.test(mfv$moral_decision, mfv_comp$moral_decision)
wilcox.test(mfv$moral_decision, mfv_comp$moral_decision, conf.int = 0.95)
=======
>>>>>>> 6df7b9f47a07458cc47cb498520b9e215ecd5abe
```
...
ggsave('../poster/LV1_pattern.png', width = 10)
