...
#   select(subID, SocialConservatism, EconomicConservatism) %>%
#   mutate(subID = as.character(subID),
#          SocialConservatism = scale(SocialConservatism),
#          EconomicConservatism = scale(EconomicConservatism))
# data <- left_join(data, secs, by = 'subID')
# 
# secs_corr <- secs %>% select(-subID)
# corr <- cor(secs_corr)
...
mfq_data <- mfq %>% select(subID, careScore, fairnessScore, loyaltyScore, authorityScore, purityScore) %>%
  mutate(subID = as.character(subID),
         careEmoScore = careScore,
         carePhysScore = careScore) %>% 
  pivot_longer(cols = c(careEmoScore, carePhysScore, fairnessScore, loyaltyScore, authorityScore, purityScore),
               names_to = 'category',
               values_to = 'MFQ') %>%
  mutate(category = factor(category, levels = c('authorityScore', 'loyaltyScore', 'purityScore', 
                                                'carePhysScore', 'careEmoScore', 'fairnessScore'),
                           labels = c('Authority', 'Loyalty', 'Purity', 'Harm-Phys', 'Harm-Emo', 'Fairness')))

data <- left_join(data, mfq_data, by=c('subID','category'))

mfq_data2 <- mfq %>% select(subID, careScore, fairnessScore, loyaltyScore, authorityScore, purityScore) %>%
  mutate(subID = as.character(subID)) %>%
  group_by(subID) %>%
  mutate(supDiffScore = (careScore + fairnessScore) - (authorityScore + loyaltyScore + purityScore)) %>% 
  select(subID, supDiffScore)

data <- left_join(data, mfq_data2, by='subID')
...
as.data.frame(emmeans(normCheck, ~ dataset*category)) %>%
  ...
  geom_pointrange(aes(ymin = lower.CL, ymax = upper.CL), position = position_dodge(0.3)) +
  scale_color_manual(values = c('gray50', 'black')) +
  labs(y = 'Estimated mean moral judgment (scaled)') +
...
ggplot(modelMoral_df, aes(x=category, y=emmean)) +
  ylim(0.98,4) +
  labs(y='Estimated mean moral judgment') +
  scale_x_discrete(labels =c('S.N.', 'Auth.', 'Loy.', 'Pur.', 'Harm-E.', 'Harm-P.', 'Fair.', 'Lib.')) +
...
modelRT_identity <- glmer(encoding_RT ~ category + (1|subID) + (1|vigfile), data = data_old, 
                 family = inverse.gaussian(link = 'identity'),
                 control=glmerControl(optimizer = 'bobyqa', 
                                     optCtrl = list(maxfun=1e9)),
                 contrasts = list(category=contr.sum))

modelRT_df <- as.data.frame(emmeans(modelRT_identity, ~ category))
ggplot(modelRT_df, aes(x=category, y=emmean)) +
  labs(y='RT (seconds)') +
  scale_x_discrete(labels =c('S.N.', 'Auth.', 'Loy.', 'Pur.', 'Harm-E.', 'Harm-P.', 'Fair.', 'Lib.')) +
  geom_violin(data=data_old, mapping=aes(y=encoding_RT, fill=category), alpha=0.85) + 
  geom_pointrange(aes(ymin=asymp.LCL, ymax=asymp.UCL), alpha=1, fatten=5, color='black') +
  #geom_dotplot(data = data_old, aes(y=encoding_RT, x=category, color=category, fill=category), 
  #             binaxis = 'y', method = 'histodot', binwidth = 0.3, dotsize = 0.8) + 
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.title = element_text(size = 18),
        legend.text = element_text(size = 18))
ggsave('figures/RT.png', dpi='retina', width = 10, height = 4)
...
# sum-coded modelMoral
modelMoral_sum <- lmer(moral_decision ~ category + (1|subID) + (1|vigfile), data=data_old, contrasts=list(category=contr.sum))
summary(modelMoral_sum)
as.data.frame(emmeans(modelMoral_sum, ~ category)) %>%
...
superordinateNull <- lmer(moral_decision ~ 1 + (1|subID) + (1|vigfile), data = data_old) 
modelMoral_sup <- lmer(moral_decision ~ category + (1|subID) + (1|vigfile), data = data_old) 
superordinateOG <- lmer(moral_decision ~ superordinate + (1|subID) + (1|vigfile), data = data_old) 
superordinateNew <- lmer(moral_decision ~ superordinate_new + (1|subID) + (1|vigfile), data = data_old) 

supCompMoral.df <- compare_performance(superordinateNull, modelMoral_sup, superordinateOG, superordinateNew, 
...
p1 <- ggplot(data = mean_centered_results_LV1, 
  ...
  labs(y = 'Brain scores', title = 'LV1: 24.4% crossblock variance') +
  scale_x_discrete(labels =c('S.N.', 'Auth.', 'Loy.', 'Pur.', 'Harm-E.', 'Harm-P.', 'Fair.', 'Lib.')) +
...
p4 <- modelLV2 %>% emmeans(~ lag * category) %>% as.data.frame() %>%
  ...
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(), 
        plot.title = element_text(hjust = 0.5, size = 28)) +
  labs(x= 'TR=2s', color='Foundation', fill='Foundation', shape='Foundation',title = 'Temporal brain scores')
...
ggsave('figures/paper/plot_NR.png', dpi = 'retina', width = 12)
```
...
supCompMemory <- compare_performance(modelMemoryNull, modelSupMemory, modelSupMemoryNew, modelMemory, bayesfactor = FALSE, rank = TRUE) %>% as.data.frame() %>%  mutate_at(c('AIC', 'BIC', 'R2_conditional', 'R2_marginal', 'ICC', 'RMSE'), funs(round(., 2)))
write.csv(supCompMemory, file = 'figures/paper/supCompMemory.csv', row.names = F)
