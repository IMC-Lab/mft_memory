...
## I exponentiated each estimate in the console to get the odds ratio; used instead of Cohen's d for effect size
emmeans(modelMemory, ~ old * category, type = 'response', lmer.df = 'satterthwaite') %>% 
  contrast(interaction = c("pairwise", old_superordinate, by = NULL), adjust='bonferroni')
  
emmeans(modelMemory, ~ old * category) %>% 
  contrast(interaction = c("pairwise", new_superordinate, by = NULL), adjust='bonferroni')
...
em_memory <- glmer(correct ~ category + angry + sad + disgusted + afraid + contemptuous + amused +
...
compare_performance(modelMemoryNull, modelMemory, em_memory, em_category_memory, em_only_memory, 
