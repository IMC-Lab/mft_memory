...
modelMoralSup <- lmer(moral_decision ~ superordinate + (1|subID) + (1|vigfile), data = em_compare, contrasts = list(superordinate=contr.sum))

modelMoralSupNew <- lmer(moral_decision ~ superordinate_new + (1|subID) + (1|vigfile), data = em_compare, contrasts = list(superordinate_new=contr.sum))

kable(compare_performance(modelMoral, em_everything, em_category, modelMoralSup, modelMoralSupNew, bayesfactor = FALSE, rank = TRUE))
```
