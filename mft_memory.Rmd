...
# plot
modelMoral_df <- as.data.frame(modelMoral.em)
...
ggsave('figures/paper/plot_LVs.png', width = 13, dpi = 400)
```


# Non-rotated PLS
```{r plot NR results, include=FALSE}
...
```{r superordinate model comparisons, include=FALSE}
...
```{r make table for sup comparison results}
kable(compare_performance(superordinateNull, modelMoral, superordinateOG, superordinateNew, bayesfactor = FALSE, rank = TRUE))
```

### empathy & emotion model comparison
```{r em model comparison, include=FALSE}
# baseline: no fixed effects
modelNull <- lmer(moral_decision ~ 1 + (1|subID) + (1|vigfile), data = em_compare, contrasts = list(category=contr.sum)) 

# standard: category as fixed effect
modelMoral <- lmer(moral_decision ~ category + (1|subID) + (1|vigfile), data = em_compare, contrasts = list(category=contr.sum))
...
emmeans(em_category, ~ category * angry) %>% eff_size(sigma=sd(em_compare$moral_decision), edf=df.residual(modelMoral), method='eff')
...
tab_model(em_category,
          #file = 'figures/paper/table_Emotion&Empathy*Foundation_2.html',
          digits = 3, 
          title = 'Table A3: Emotion & Empathy*Foundation summary',
          show.ci = 0.95, show.stat = TRUE, show.p = TRUE, show.ngroups = TRUE, show.df = TRUE,
          p.val = 'satterthwaite', 
          strings = c(stat = 't value', ci = 'CI (95%)', pred = 'Factors', p = 'p value'),
          dv.labels = 'moral judgment', 
          ...
          pred.labels = c('Intercept',
                          ...
                          'Anger', 'Amusement', 'Sadness', 'Fear','Contempt','Disgust', 'Personal Distress', 'Empathic Concern', 'Perspective Taking', 'Fantasy',
                          'Social Norms*Anger', 'Authority*Anger', 'Loyalty*Anger', 'Purity*Anger', 'Harm-Emo*Anger', 'Harm-Phys*Anger', 'Fairness*Anger',
                          'Social Norms*Amusement','Authority*Amusement','Loyalty*Amusement','Purity*Amusement','Harm-Emo*Amusement','Harm-Phys*Amusement','Fairness*Amusement',
                          'Social Norms*Sadness', 'Authority*Sadness', 'Loyalty*Sadness', 'Purity*Sadness', 'Harm-Emo*Sadness', 'Harm-Phys*Sadness', 'Fairness*Sadness',
                          'Social Norms*Fear', 'Authority*Fear', 'Loyalty*Fear', 'Purity*Fear', 'Harm-Emo*Fear', 'Harm-Phys*Fear', 'Fairness*Fear',
                          'Social Norms*Contempt', 'Authority*Contempt', 'Loyalty*Contempt', 'Purity*Contempt', 'Harm-Emo*Contempt', 'Harm-Phys*Contempt', 'Fairness*Contempt',
                          'Social Norms*Disgust', 'Authority*Disgust', 'Loyalty*Disgust', 'Purity*Disgust', 'Harm-Emo*Disgust', 'Harm-Phys*Disgust', 'Fairness*Disgust',
                          'Social Norms*Personal Distress','Authority*Personal Distress','Loyalty*Personal Distress','Purity*Personal Distress',
                          'Harm-Emo*Personal Distress','Harm-Phys*Personal Distress','Fairness*Personal Distress',
                          'Social Norms*Empathic Concern','Authority*Empathic Concern','Loyalty*Empathic Concern','Purity*Empathic Concern',
                          'Harm-Emo*Empathic Concern','Harm-Phys*Empathic Concern','Fairness*Empathic Concern',
                          'Social Norms*Perspective Taking', 'Authority*Perspective Taking', 'Loyalty*Perspective Taking', 'Purity*Perspective Taking',
                          'Harm-Emo*Perspective Taking', 'Harm-Phys*Perspective Taking', 'Fairness*Perspective Taking',
                          'Social Norms*Fantasy', 'Authority*Fantasy', 'Loyalty*Fantasy','Purity*Fantasy','Harm-Emo*Fantasy','Harm-Phys*Fantasy','Fairness*Fantasy'))


em_categoryCategory <- emmeans(em_category, ~ category, at=em_category.cov)
eff_size(em_categoryCategory, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'eff')

em_categoryAngry <- emmeans(em_category, ~ angry, at=list_modify(em_category.cov, angry=c(0,1)))
eff_size(em_categoryAngry, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryAmused <- emmeans(em_category, ~ amused, at=list_modify(em_category.cov, amused=c(0,1)))
eff_size(em_categoryAmused, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categorySad <- emmeans(em_category, ~ sad, at=list_modify(em_category.cov, sad=c(0,1)))
eff_size(em_categorySad, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryFear <- emmeans(em_category, ~ afraid, at=list_modify(em_category.cov, afraid=c(0,1)))
eff_size(em_categoryFear, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryContempt <- emmeans(em_category, ~ contemptuous, at=list_modify(em_category.cov, contemptuous=c(0,1)))
eff_size(em_categoryContempt, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryDisgust <- emmeans(em_category, ~ disgusted, at=list_modify(em_category.cov, disgusted=c(0,1)))
eff_size(em_categoryDisgust, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryPD <- emmeans(em_category, ~ PersonalDistressScore, at=list_modify(em_category.cov, PersonalDistressScore=c(0,1)))
eff_size(em_categoryPD, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryEC <- emmeans(em_category, ~ EmpathicConcernScore, at=list_modify(em_category.cov, EmpathicConcernScore=c(0,1)))
eff_size(em_categoryEC, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryPT <- emmeans(em_category, ~ PerspectiveTakingScore, at=list_modify(em_category.cov, PerspectiveTakingScore=c(0,1)))
eff_size(em_categoryPT, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryF <- emmeans(em_category, ~ FantasyScore, at=list_modify(em_category.cov, FantasyScore=c(0,1)))
eff_size(em_categoryF, sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'revpairwise')

em_categoryAngryInt <- emmeans(em_category, ~ angry * category, at=list_modify(em_category.cov, angry=c(0,1)))
em_categoryAngryInt %>% contrast(interaction=c('revpairwise', 'eff')) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'identity')

em_categoryAmusedInt <- emmeans(em_category, ~ amused * category, at=list_modify(em_category.cov, amused=c(0,1)))
em_categoryAmusedInt %>% contrast(interaction=c('revpairwise', 'eff')) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'identity')

em_categorySadInt <- emmeans(em_category, ~ sad * category, at=list_modify(em_category.cov, sad=c(0,1)))
em_categorySadInt %>% contrast(interaction=c('revpairwise', 'eff')) %>%
  eff_size(sigma = sd(em_compare$moral_decision), edf = df.residual(em_category), method = 'identity')
...
tab_model(modelMemory, 
          ...
          pred.labels = c('Intercept (Social Norms)', 
                          'Authority', 'Loyalty', 'Purity', 'Harm-Emo', 'Harm-Phys', 'Fairness', 'Liberty',
                          'New', 'Authority*New', 'Loyalty*New', 'Purity*New', 'Harm-Emo*New', 'Harm-Phys*New',
                          'Fairness*New', 'Liberty*New'))

# compute effect size 
modelMemory.emt <- emmeans(modelMemory, ~ old*category, type='response')

modelMemory %>% emmeans( ~ old | category) %>%
  eff_size(edf = df.residual(modelMemory), sigma = sd(data$correct), method = 'pairwise')

modelMemory.emt <- emmeans(modelMemory, ~ category)
eff_size(modelMemory.emt, edf = df.residual(modelMemory), sigma = sd(data$correct), method = 'identity')

modelMemoryOld.emt <- emmeans(modelMemory, ~old)
eff_size(modelMemoryOld.emt, edf = df.residual(modelMemory), sigma = sd(data$correct), method = 'pairwise')
